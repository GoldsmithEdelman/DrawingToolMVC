...
public class MyBMPFile extends Component
{
    ...
    static public void write(String filename, Image image) throws IOException
    {
        ...
        reader.saveBitmap(filename, image, image.getWidth(reader),
                image.getHeight(reader));
    }
    ...
    private void saveBitmap(String parFilename, Image parImage, int parWidth,
    ...
    private int intFromLSBShort(byte raw[], int index)
    {
        ...
        return (((int) raw[index + 1] & 0xff) << 8) | (int) raw[index] & 0xff;
    }
    ...
    private byte[] readPixelData(FileInputStream fs, String filename,
            int compression, int width, int height, int size) throws IOException
    {
        ...
        {
            throw new IOException("BMPFile.read('" + filename
    ...
    private Image loadBitmap(String sfile) throws IOException
    {
        ...
        if ((char) bf[0] != 'B' || (char) bf[1] != 'M') throw new IOException(
                "BMPFile.read('" + sfile + "'): File isn't a BMP image.");
        ...
        else if (nbitcount == 8)
        {
            ...
            byte bdata[] = readPixelData(fs, sfile, ncompression,
                    bytes_per_line, nheight, nsizeimage);
            nindex8 = 0;
            for (int j8 = 0; j8 < nheight; j8++)
            {
                for (int i8 = 0; i8 < nwidth; i8++)
                {
                    ndata8[nwidth * (nheight - j8 - 1)
        ...
        else
        {
            throw new IOException(
                    "BMPFile.read('" + sfile + "'): Unsupported pixel depth.");
        }
